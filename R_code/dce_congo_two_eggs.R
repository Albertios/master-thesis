...
for (i in 17:length(years_Congo)) {
  df <- get_year_Congo(years_Congo[20])
  
  year <- as.numeric(years_Congo[20])
    ...
    if (nrow(unique_egg_one) > 2 ) {
      ...
      #     h3(paste0("Congo Year ", year)),
          leaflet(egg_points) %>% addTiles() %>%
            ...
            #             color = "black") %>%
            # addPolygons(lng = concave_ellipse[,1],
            #             lat = concave_ellipse[,2],
            #             color = "red") %>%
          # addCircleMarkers(lng = combined_ellipses[,1],
          #               lat = combined_ellipses[,2],
          #               color = "blue",
          #               radius = ~ ifelse(type == "5", 1, 1)) %>%
            addCircleMarkers(
              data = df,
              lat = ~ LATITUDE,
              lng = ~ LONGITUDE,
